...
import {
  collectDataPathsFromDatamodels,
  collectDataPathsFromObject,
} from "../components/data-path-autocomplete.mjs";
import {
  AURA_POSITIONS,
...
import {
  isSequencerActive,
  isTerrainHeightToolsActive,
  partialEqual,
} from "../utils/misc-utils.mjs";
...
export class AuraConfigApplication extends ApplicationV2 {
  ...
  constructor(
    ...
    this.#visibilityMode = this.#getVisibilityMode(
      aura.ownerVisibility,
      aura.nonOwnerVisibility,
    );
    ...
    this.#datapathAutocompleteSuggestions = radiusContext?.actor
      ...
      : [
          ...collectDataPathsFromDatamodels(CONFIG.Actor.dataModels, {
            prefix: "actor",
          }),
          ...collectDataPathsFromDatamodels(CONFIG.Item.dataModels, {
            prefix: "item",
          }),
        ];
  }
        ...
        ${this.#tabs.map((tab, index) =>
          tab.hidden
            ...
            : html`
                <a
                  class=${classMap({
                    active: this.#selectedTabIndex === index,
                  })}
                  @click=${() => this.#setSelectedTab(index)}
                >
                  <span class="gaa-vertical-tabs-icon"
                    ><i class=${tab.icon}></i
                  ></span>
                  <span>${tab.name}</span>
                </a>
      ...
      ${when(
        ...
        () => html`
          <div
            class="gaa-popover"
            @click=${(e) =>
              !e.target.closest(".gaa-popover-content") &&
              this.#setAlternateContent(null, { render: true })}
          >
            <div class="gaa-popover-content">${this.#alternateContent}</div>
          </div>
        `,
      )}
    `;
  ...
  #geometryTab = () => {
    const radiusIsInvalidPath =
      typeof calculateAuraRadius(this.#aura.radius, this.#radiusContext) !==
      "number";
    const innerRadiusIsInvalidPath =
      this.#aura.innerRadius !== "" && // innerRadius is optional
      typeof calculateAuraRadius(
        this.#aura.innerRadius,
        this.#radiusContext,
      ) !== "number";
        ...
        <div class="form-group">
          ...
          ${when(
            ...
            () => html`
              <div
                class="hint"
                style="text-align: right; color: var(--color-level-error);"
              >
                ${l("GRIDAWAREAURAS.UnresolvedRadiusConfigDialogWarning")}
              </div>
            `,
          )}
        </div>
        ...
        <div class="form-group">
          ...
          <div class="form-fields">
            <select name="position" ?disabled=${this.#disabled}>
              ${selectOptions(AURA_POSITIONS, {
                selected: this.#aura.position,
              })}
            </select>
          </div>
    ...
    return html`
      <div class="standard-form">
          ...
          <div class="form-fields">
            ...
            >
              ${selectOptions(LINE_TYPES, {
                ...
                labelSelector: ([name]) =>
                  `GRIDAWAREAURAS.LineType${name.titleCase()}`,
        ...
        <div class=${classMap({ "form-group": true, hidden: isNone })}>
          <label
            >${l("DRAWING.LineWidth")} <span class="units">(px)</span></label
          >
          <div class="form-fields">
            <input
              type="number"
              name="lineWidth"
              .value=${this.#aura.lineWidth}
              required
              min="0"
              step="1"
              ?disabled=${this.#disabled}
        ...
        <div
          class=${classMap({
            "form-group": true,
            hidden: isNone || !!this.#aura.lineColorAnimation,
          })}
        >
        ...
        ${when(
          this.#aura.lineColorAnimation && !isNone,
          () => html`
            <div class="form-group">
              <label>${l("DRAWING.StrokeColor")}</label>
              <div class="form-fields">
                ...
                <button
                  ...
                  @click=${() =>
                    this.#setAuraProperty("lineColorAnimation", null)}
                  ?disabled=${this.#disabled}
                >
                  <i class="fas fa-sparkles m-0"></i>
                </button>
        ...
        <div class=${classMap({ "form-group": true, hidden: !isDashed })}>
          ...
          <div class="form-fields">
            <input
              type="number"
              name="lineDashOffsetAnimation"
              placeholder="Dash"
              .value=${this.#aura.lineDashOffsetAnimation}
              required
              step="1"
              ?disabled=${this.#disabled}
            />
    ...
    return html`
      <div class="standard-form">
          ...
          <div class="form-fields">
            ...
            >
              ${selectOptions(CONST.DRAWING_FILL_TYPES, {
                ...
                labelSelector: ([name]) =>
                  `DRAWING.FillType${name.titleCase()}`,
                valueSelector: ([, value]) => value,
              })}
            </select>
          </div>
        ...
        <div
          class=${classMap({
            "form-group": true,
            hidden: isNone || !!this.#aura.fillColorAnimation,
          })}
        >
        ...
        ${when(
          !isNone && this.#aura.fillColorAnimation,
          () => html`
            <div class="form-group">
              <label>${l("DRAWING.FillColor")}</label>
              <div class="form-fields">
                ...
                <button
                  ...
                  data-tooltip="Disable animation"
                  @click=${() =>
                    this.#setAuraProperty("fillColorAnimation", null)}
                  ?disabled=${this.#disabled}
                >
                  <i class="fas fa-sparkles m-0"></i>
                </button>
        ...
        <div
          class=${classMap({
            "form-group": true,
            hidden: !isPattern || !!this.#aura.fillTextureOffsetAnimation,
          })}
        >
          ...
          <div class="form-fields">
            <input
              type="number"
              name="fillTextureOffset.x"
              placeholder="x"
              .value=${this.#aura.fillTextureOffset.x}
              required
              ?disabled=${this.#disabled}
            />
            <input
              type="number"
              name="fillTextureOffset.y"
              placeholder="y"
              .value=${this.#aura.fillTextureOffset.y}
              required
              ?disabled=${this.#disabled}
            />
            <button
              ...
              data-tooltip="Enable animation"
              @click=${() =>
                this.#setAuraProperty("fillTextureOffsetAnimation", {
                  x: 0,
                  y: 0,
                })}
              ?disabled=${this.#disabled}
        ...
        ${when(
          isPattern && this.#aura.fillTextureOffsetAnimation,
          () => html`
            <div class="form-group">
              ...
              <div class="form-fields">
                <input
                  type="number"
                  name="fillTextureOffsetAnimation.x"
                  placeholder="x"
                  .value=${this.#aura.fillTextureOffsetAnimation.x}
                  required
                  ?disabled=${this.#disabled}
                />
                <input
                  type="number"
                  name="fillTextureOffsetAnimation.y"
                  placeholder="y"
                  .value=${this.#aura.fillTextureOffsetAnimation.y}
                  required
                  ?disabled=${this.#disabled}
                />
                <button
                  ...
                  data-tooltip="Disable animation"
                  @click=${() =>
                    this.#setAuraProperty("fillTextureOffsetAnimation", null)}
                  ?disabled=${this.#disabled}
                >
                  <i class="fas fa-sparkles m-0"></i>
                </button>
        ...
        <div class=${classMap({ "form-group": true, hidden: !isPattern })}>
          ...
          <div class="form-fields">
            <input
              type="number"
              name="fillTextureScale.x"
              placeholder="x"
              .value=${this.#aura.fillTextureScale.x}
              required
              ?disabled=${this.#disabled}
            />
            <input
              type="number"
              name="fillTextureScale.y"
              placeholder="y"
              .value=${this.#aura.fillTextureScale.y}
              required
              ?disabled=${this.#disabled}
            />
          </div>
  ...
  #visibilityTab = () => html`
    <div class="standard-form">
      <div class="form-group">
        ...
        <div class="form-fields">
          <select
            name="visibilityMode"
            ?disabled=${this.#disabled}
            @change=${this.#setVisibilityMode}
          >
            ${selectOptions(AURA_VISIBILITY_MODES, {
              selected: this.#visibilityMode,
            })}
          </select>
        </div>
      </div>
      ...
      <fieldset
        class=${classMap({
          disabled: this.#disabled,
          hidden: this.#visibilityMode !== "CUSTOM",
        })}
      ...
      >
        ...
        <p class="hint" style="margin-top: 0;">
          Specify under which states the aura should be visible to owners and
          non-owners. When multiple states are appliable, the aura is visible
          when ANY applicable state is checked.
        </p>
        ...
        <div class="visibility-grid">
          ...
          <div class="visibility-row">
            ...
            <input
              type="checkbox"
              class="owner"
              name="ownerVisibility.default"
              .checked=${this.#aura.ownerVisibility.default}
            />
            <input
              type="checkbox"
              class="nonowner"
              name="nonOwnerVisibility.default"
              .checked=${this.#aura.nonOwnerVisibility.default}
            />
          </div>
          ...
          <div class="visibility-row">
            ...
            <input
              type="checkbox"
              class="owner"
              name="ownerVisibility.hovered"
              .checked=${this.#aura.ownerVisibility.hovered}
            />
            <input
              type="checkbox"
              class="nonowner"
              name="nonOwnerVisibility.hovered"
              .checked=${this.#aura.nonOwnerVisibility.hovered}
            />
          </div>
          ...
          <div class="visibility-row">
            ...
            <input
              type="checkbox"
              class="owner"
              name="ownerVisibility.controlled"
              .checked=${this.#aura.ownerVisibility.controlled}
            />
          </div>
          ...
          <div class="visibility-row">
            ...
            <input
              type="checkbox"
              class="owner"
              name="ownerVisibility.dragging"
              .checked=${this.#aura.ownerVisibility.dragging}
            />
          </div>
          ...
          <div class="visibility-row">
            ...
            <input
              type="checkbox"
              class="owner"
              name="ownerVisibility.targeted"
              .checked=${this.#aura.ownerVisibility.targeted}
            />
            <input
              type="checkbox"
              class="nonowner"
              name="nonOwnerVisibility.targeted"
              .checked=${this.#aura.nonOwnerVisibility.targeted}
            />
          </div>
          ...
          <div class="visibility-row">
            ...
            <p class="hint">
              When the token has its turn in the combat tracker.
            </p>
            <input
              type="checkbox"
              class="owner"
              name="ownerVisibility.turn"
              .checked=${this.#aura.ownerVisibility.turn}
            />
            <input
              type="checkbox"
              class="nonowner"
              name="nonOwnerVisibility.turn"
              .checked=${this.#aura.nonOwnerVisibility.turn}
            />
          </div>
        </div>
      </fieldset>
  ...
  #automationEffectTab = () => {
    const effectsEnabled = game.settings.get(
      MODULE_NAME,
      ENABLE_EFFECT_AUTOMATION_SETTING,
    );

    /** @type {(e: Event, effect: EffectConfig, idx: number) => void} */
    const openItemContextMenu = (e, effect, idx) =>
      ContextMenu.open(e, [
        {
          label: "Edit",
          icon: "fas fa-edit",
    ...
    return html`
      ${when(
        ...
        () => html`
          <p class="alert" role="alert">
            Effect automation is not turned on for this world. GMs can configure
            this in the settings.
          </p>
        `,
      )}
      ${when(
        this.#aura.effects.length,
        () =>
          html`<ul class="automated-item-list">
            ${this.#aura.effects.map(
              (effect, idx) => html`
                <li @contextmenu=${(e) => openItemContextMenu(e, effect, idx)}>
                  ...
                  ${when(
                    !this.#disabled && effectsEnabled,
                    () => html`
                      <a
                        class="menu-button"
                        @click=${(e) => openItemContextMenu(e, effect, idx)}
                      >
      ...
      ${when(
        effectsEnabled && this.#aura.effects.length === 0,
        () => html`
      ...
      <div class="automated-item-list-create-button">
        <button
          @click=${this.#createEffect}
          ?disabled=${this.#disabled || !effectsEnabled}
        >
  ...
  #effectEditNestedDialog = (editingEffectIndex) => {
    ...
    return html`
      <form
        class="standard-form"
        @submit=${(e) =>
          this.#updateArrayItem(e, this.#aura.effects, editingEffectIndex)}
      >
        <div class="form-group">
          <label>Effect</label>
          <div class="form-fields">
        ...
        <div class="form-group">
          ...
          <div class="form-fields">
            <select name="targetTokens" ?disabled=${this.#disabled}>
              ${selectOptions(listAuraTargetFilters(), {
                selected: editingEffect.targetTokens,
              })}
            </select>
          </div>
        </div>
        ...
        <div class="flexrow">
          ${when(
            ...
            () => html`
              <button
                type="button"
                @click=${() => this.#deleteEffect(editingEffectIndex)}
              >
  ...
  #automationMacroTab = () => {
    const macrosEnabled = game.settings.get(
      MODULE_NAME,
      ENABLE_MACRO_AUTOMATION_SETTING,
    );

    /** @type {(e: Event, macro: MacroConfig, idx: number) => void} */
    const openItemContextMenu = (e, macro, idx) =>
      ContextMenu.open(e, [
        {
          label: "Edit",
          icon: "fas fa-edit",
    ...
    return html`
      ${when(
        ...
        () => html`
          <p class="alert" role="alert">
            Macro automation is not turned on for this world. GMs can configure
            this in the settings.
          </p>
        `,
      )}
      ${when(
        this.#aura.macros.length,
        () =>
          html`<ul class="automated-item-list">
            ${this.#aura.macros.map(
              (macro, idx) => html`
                <li @contextmenu=${(e) => openItemContextMenu(e, macro, idx)}>
                  <div class="flexcol">
                    <span
                      ><strong
                        >${game.macros.get(macro.macroId)?.name ??
                        l("None")}</strong
                      ></span
                    >
                  ...
                  ${when(
                    !this.#disabled && macrosEnabled,
                    () => html`
                      <a
                        class="menu-button"
                        @click=${(e) => openItemContextMenu(e, macro, idx)}
                      >
                        <i class="fas fa-ellipsis-vertical"></i>
                      </a>
                    `,
      ...
      ${when(
        macrosEnabled && this.#aura.macros.length === 0,
        () => html` <p class="hint text-center">No macros configured.</p> `,
      )}

      <div class="automated-item-list-create-button">
        <button
          @click=${this.#createMacro}
          ?disabled=${this.#disabled || !macrosEnabled}
        >
          <i class="fas fa-plus"></i>
          Create Macro
        </button>
  ...
  #macroEditNestedDialog = (macroIndex) => {
    ...
    return html`
      <form
        ...
        @drop=${this.#disabled
          ? nothing
          : (e) => this.#onMacroDrop(e, macroInputRef)}
        @submit=${(e) =>
      ...
      >
        <div class="form-group">
          ...
          <div class="form-fields flexcol">
            <input
              type="text"
              name="macroId"
              value=${editingMacro.macroId}
              ?disabled=${this.#disabled}
              ${macroInputRef.value}
            />
            <p class="hint">
              Enter a macro's ID, or drag and drop it onto the textbox.
            </p>
          </div>
        </div>
        ...
        <div class="form-group">
          ...
          <div class="form-fields">
            <select name="targetTokens" ?disabled=${this.#disabled}>
              ${selectOptions(listAuraTargetFilters(), {
                selected: editingMacro.targetTokens,
              })}
            </select>
          </div>
        </div>
        ...
        <div class="flexrow">
          ${when(
            ...
            () => html`
              <button
                type="button"
                @click=${() => this.#deleteMacro(macroIndex)}
              >
                <i class="fas fa-trash"></i> ${l("Delete")}
              </button>
              <button type="submit">
  ...
  #automationSequencerTab = () => {
    ...
    const sequencerEnabled =
      game.settings.get("sequencer", "permissions-effect-create") === 0;

    /** @type {(e: Event, sequence: SequencerEffectConfig, idx: number) => void} */
    const openItemContextMenu = (e, sequence, idx) =>
      ContextMenu.open(e, [
        {
          label: "Edit",
          icon: "fas fa-edit",
    ...
    return html`
      ${when(
        ...
        () => html`
          <p class="alert" role="alert">
            Sequencer integration requires players to have permission to create
            effects. GMs can configure this in the Sequencer settings.
          </p>
        `,
      )}
      ${when(
        this.#aura.sequencerEffects.length,
        () =>
          html`<ul class="automated-item-list">
            ${this.#aura.sequencerEffects.map(
              (sequence, idx) => html`
                <li
                  @contextmenu=${(e) => openItemContextMenu(e, sequence, idx)}
                >
                  <div class="flexcol">
                    <span
                      ><strong
                        >${sequence.effectPath?.length
                          ? sequence.effectPath
                          : "- No effect selected -"}</strong
                      ></span
                    >
                    <span
                      ><em
                        >${l(SEQUENCE_TRIGGERS[sequence.trigger] ?? "")}</em
                      ></span
                    >
                    <span
                      ><em
                        >${l(SEQUENCE_POSITIONS[sequence.position] ?? "")}</em
                      ></span
                    >
                  </div>
                  ${when(
                    !this.#disabled && sequencerEnabled,
                    () => html`
                      <a
                        class="menu-button"
                        @click=${(e) => openItemContextMenu(e, sequence, idx)}
                      >
                        <i class="fas fa-ellipsis-vertical"></i>
                      </a>
                    `,
      ...
      ${when(
        sequencerEnabled && this.#aura.sequencerEffects.length === 0,
        () => html`
      ...
      <div class="automated-item-list-create-button">
        <button
          @click=${this.#createSequence}
          ?disabled=${this.#disabled || !sequencerEnabled}
        >
          <i class="fas fa-plus"></i>
          Create Sequence
        </button>
  ...
  #sequenceEditNestedDialog = (sequenceIndex) => {
    ...
    return html`
      <form
        class="flexcol"
        @submit=${(e) =>
          this.#updateArrayItem(e, this.#aura.sequencerEffects, sequenceIndex)}
      >
        <div
          class="standard-form"
          style="flex: 1; overflow-y: scroll; padding-right: 1rem;"
        >
          ...
          <div class="form-group">
            ...
            <div class="form-fields">
              <input
                type="text"
                name="effectPath"
                value=${effect.effectPath}
                ?disabled=${this.#disabled}
              />
              <button
                type="button"
                @click=${() => Sequencer.DatabaseViewer.show()}
              >
                <i class="fas fa-database"></i>
              </button>
            </div>
            <p class="hint">
              The Sequencer file to play. Can be a filepath, wildcard filepath
              or database path.
            </p>
          </div>
          ...
          <div class="form-group">
            ...
            <div class="form-fields">
              <select name="trigger" ?disabled=${this.#disabled}>
                ${selectOptions(SEQUENCE_TRIGGERS, {
                  selected: effect.trigger,
                })}
              </select>
            </div>
          </div>
          ...
          <div class="form-group">
            ...
            <div class="form-fields">
              <select name="position" ?disabled=${this.#disabled}>
                ${selectOptions(SEQUENCE_POSITIONS, {
                  selected: effect.position,
                })}
              </select>
            </div>
          </div>
          ...
          <div class="form-group">
            ...
            <p class="hint">
              How many times the effect should play, and how long between
              repeats.
            </p>
          </div>
          ...
          <div class="form-group">
            ...
            <div class="form-fields">
              <input
                type="number"
                name="fadeInDuration"
                value=${effect.fadeInDuration}
                min="0"
                ?disabled=${this.#disabled}
              />
              <span class="units" style="margin-right: 0.75rem">ms</span>
              <select name="fadeInEasing" ?disabled=${this.#disabled}>
                ${selectOptions(EASING_FUNCTIONS, {
                  selected: effect.fadeInEasing,
                  labelSelector: ([, name]) => `GRIDAWAREAURAS.${name}`,
                })}
              </select>
            </div>
          </div>
          ...
          <div class="form-group">
            ...
            <div class="form-fields">
              <input
                type="number"
                name="scaleOutScale"
                value=${effect.scaleOutScale}
                ?disabled=${this.#disabled}
              />
              <span class="units" style="margin-right: 0.75rem">x</span>
              <input
                type="number"
                name="scaleOutDuration"
                value=${effect.scaleOutDuration}
                min="0"
                step="0.01"
                ?disabled=${this.#disabled}
              />
              ...
              >
                ${selectOptions(EASING_FUNCTIONS, {
                  selected: effect.scaleOutEasing,
                  labelSelector: ([, name]) => `GRIDAWAREAURAS.${name}`,
                })}
              </select>
            </div>
          ...
          <div class="form-group">
            ...
            <div class="form-fields">
              <input
                type="checkbox"
                name="belowTokens"
                ?checked=${effect.belowTokens}
                ?disabled=${this.#disabled}
              />
            </div>
            <p class="hint">
              Note that auras render at the same Z-index as tokens, so this also
              draws the effect below auras.
            </p>
          </div>
        </div>
        ...
        <div class="flexrow" style="margin-top: 1rem">
          ${when(
            ...
            () => html`
              <button
                type="button"
                @click=${() => this.#deleteSequence(sequenceIndex)}
              >
                <i class="fas fa-trash"></i> ${l("Delete")}
              </button>
              <button type="submit">
  ...
  #automationThtTab = () => html`
    <div class="standard-form">
      <div class="form-group">
        ...
        <div class="form-fields">
          <select
            name="terrainHeightTools.rulerOnDrag"
            ?disabled=${this.#disabled}
          >
            ${selectOptions(THT_RULER_ON_DRAG_MODES, {
              selected: this.#aura.terrainHeightTools.rulerOnDrag,
            })}
          </select>
        </div>
      </div>
      ...
      <div
        class=${classMap({
          "form-group": true,
          hidden: this.#aura.terrainHeightTools.rulerOnDrag === "NONE",
        })}
      >
        <label>Target Tokens</label>
        <div class="form-fields">
          <select
            name="terrainHeightTools.targetTokens"
            ?disabled=${this.#disabled}
          >
            ${selectOptions(listAuraTargetFilters(), {
              selected: this.#aura.terrainHeightTools.targetTokens,
            })}
          </select>
        </div>
      </div>
  ...
  /** @type {(e: Event) => void} */
  #valueChange = (e) => {
    const name = e.target.name?.length
      ? e.target.name
      : e.target.closest("[name]")?.name;
  ...
  #setVisibilityMode = (e) => {
    ...
    if (!isCustom) {
      ...
      Object.entries(preset.owner).forEach(
        ([key, value]) => (this.#aura.ownerVisibility[key] = value),
      );
      Object.entries(preset.nonOwner).forEach(
        ([key, value]) => (this.#aura.nonOwnerVisibility[key] = value),
      );
  ...
  #getVisibilityMode(ownerVisibility, nonOwnerVisibility) {
    for (const [mode, modeConfig] of Object.entries(
      ...
      if (
        partialEqual(ownerVisibility, modeConfig.owner) &&
        partialEqual(nonOwnerVisibility, modeConfig.nonOwner)
      )
        return mode;
    }
    return "CUSTOM";
  ...
  #createEffect = (source) => {
    ...
    this.#setAlternateContent(
      this.#effectEditNestedDialog(this.#aura.effects.length - 1),
      { title: "Edit Effect" },
    );
    this.#auraUpdated();
  };
  ...
  #editEffect = (effectIndex) => {
    this.#setAlternateContent(this.#effectEditNestedDialog(effectIndex), {
      title: "Edit Effect",
      render: true,
    });
  };
  ...
  #deleteEffect = (effectIndex) => {
    this.#aura.effects = this.#aura.effects.filter(
  ...
  #createMacro = (source) => {
    ...
    this.#setAlternateContent(
      this.#macroEditNestedDialog(this.#aura.macros.length - 1),
      { title: "Edit Macro" },
    );
    this.#auraUpdated();
  };
  ...
  #editMacro = (macroIndex) => {
    this.#setAlternateContent(this.#macroEditNestedDialog(macroIndex), {
      title: "Edit Macro",
      render: true,
    });
  };
  ...
  #deleteMacro = (macroIndex) => {
    this.#aura.macros = this.#aura.macros.filter(
      (_, idx) => idx !== macroIndex,
    );
    this.#setAlternateContent(null);
    this.#auraUpdated();
  };
  ...
  #editSequence = (sequenceIndex) => {
    this.#setAlternateContent(this.#sequenceEditNestedDialog(sequenceIndex), {
      title: "Edit Sequencer Effect",
      render: true,
    });
  };
  ...
  #deleteSequence = (sequenceIndex) => {
    this.#aura.sequencerEffects = this.#aura.sequencerEffects.filter(
      (_, idx) => idx !== sequenceIndex,
    );
    this.#setAlternateContent(null);
    this.#auraUpdated();
  };
  ...
  #setAlternateContent(content, { title, render = false } = {}) {
    ...
    this.element.querySelector(".window-title").innerText =
      "Aura Configuration" + (title?.length ? ` :: ${title}` : "");
  ...
  async close(options) {
    ...
    if (options?.callOnClose !== false) this.#onClose?.();
  ...
  _updateFrame(options) {
    ...
    const buttons = html`
      <button
        ...
        data-tooltip=${`${aura}: ${this.#aura.id}`}
