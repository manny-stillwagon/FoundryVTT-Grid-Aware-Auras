...
import {
  tokenConfigClose,
  v12TokenConfigRenderInner,
  v13TokenConfigRender,
} from "./applications/token-aura-config.mjs";
import { setupAutomation } from "./automation/index.mjs";
import {
  DOCUMENT_AURAS_FLAG,
  END_MOVE_INSIDE_AURA_HOOK,
  MODULE_NAME,
  SOCKET_NAME,
  START_MOVE_INSIDE_AURA_HOOK,
  TOGGLE_EFFECT_FUNC,
} from "./consts.mjs";
...
Hooks.once("init", () => {
  ...
  CONFIG.Canvas.layers.gaaAuraLayer = {
    group: "interface",
    layerClass: AuraLayer,
  };
...
Hooks.once("ready", () => {
  switch (game.release.generation) {
    case 12: {
      ...
      libWrapper.register(
        MODULE_NAME,
        "TokenConfig.prototype._renderInner",
        v12TokenConfigRenderInner,
        libWrapper.WRAPPER,
      );
    ...
    case 13: {
      ...
      const patchTokenConfig = (cls) => {
        if (!(cls.prototype instanceof foundry.applications.api.ApplicationV2))
          return;
        ...
        cls.PARTS.gridAwareAuras = {
          template: `modules/${MODULE_NAME}/templates/v13-token-config-tab.hbs`,
          scrollable: [],
        };
    ...
    case 14: {
      // Currently just a copy of v13 until I find something broken
      const patchedTypes = new Set();
      const patchTokenConfig = (cls) => {
        if (!(cls.prototype instanceof foundry.applications.api.ApplicationV2))
          return;
  ...
  game.socket.on(SOCKET_NAME, ({ func, runOn, ...args }) => {
    if (runOn?.length > 0 && runOn !== game.userId) return;
...
Hooks.on("preCreateToken", (tokenDocument, data) => {
  ...
  const applicablePresets = getPresets().filter((p) =>
    p.applyToNew.includes(actor.type),
  );
  for (const preset of applicablePresets) {
    ...
    if (
      !auras.some(
        (a) =>
          a.name.localeCompare(preset.config.name, undefined, {
            sensitivity: "accent",
          }) === 0,
      )
    )
...
Hooks.on("updateToken", (tokenDocument, delta, _options, userId) => {
  ...
  const startingAuras = isMovementUpdate
    ? AuraLayer.current._auraManager.getAurasContainingToken(token)
    : [];
  ...
  if (
    Object.keys(foundry.utils.flattenObject(delta)).some((k) =>
      watchedTokenProperties.includes(k),
    )
  ) {
  ...
  const endingAuras = isMovementUpdate
    ? AuraLayer.current._auraManager.getAurasContainingToken(token)
    : [];
  for (const aura of endingAuras) {
    const startedInside = startingAuras.some(
      (a) => a.aura === aura.aura && a.parent === aura.parent,
    );
    Hooks.callAll(
      END_MOVE_INSIDE_AURA_HOOK,
      token,
      aura.parent,
      aura.aura.config,
      { startedInside, startPosition, userId },
    );
  }
});
  ...
  if (refreshPosition || refreshVisibility) {
    ...
    if (token.isPreview) {
      ...
      AuraLayer.current?._testCollisionsForToken(token, {
        useActualPosition: true,
      });
    } else {
      AuraLayer.current?._updateAuraGraphics({
        token,
        updatePosition: !!refreshPosition,
      });
    }
  }
});
...
Hooks.on("canvasTearDown", () => {
  if (AuraLayer.current) AuraLayer.current._isTearingDown = true;
});
