...
export class HexagonalAuraGeometry {
  ...
  constructor(width, height, radius, shape, isColumnar, gridSize) {
    ...
    this.#points = HexagonalAuraGeometry.#getPoints(
      width,
      height,
      radius,
      shape,
    ...
    ({
      collidableEdges: this.#collidableEdges,
      boundingBox: this.#boundingBox,
    } = HexagonalAuraGeometry.#getCollisionTestData(this.#points));
  ...
  isInside(
    token,
    { auraOffset = { x: 0, y: 0 }, tokenAltPosition, mode = "partial" } = {},
  ) {
    const { width, height, shape } = token.document;
    const { x, y } = tokenAltPosition ?? token;
    const points = getSpacesUnderHexToken(
      x,
      y,
      width,
      height,
    ...
    return points[mode === "total" ? "every" : "some"]((p) =>
      this._isPointInside(p.x - auraOffset.x, p.y - auraOffset.y),
    );
  }
  ...
  *getPath() {
  ...
  static #getPoints(width, height, radius, shape, isColumnar) {
    ...
    // hex border for it
    if (width < 1 && width === height) width = height = 1;
  ...
  static #getCollisionTestData(points) {
    ...
    edges.sort((a, b) =>
      a.p1.y === b.p1.y ? a.p1.x - b.p1.x : a.p1.y - b.p1.y,
    );
...
const generateHexBorder = cacheReturn(
  ...
  function (
    ...
    function* generateSide(sideLength, angle) {
      ...
      const dx1 =
        Math.cos(angle + RADIANS_30) * UNIT_SIDE_LENGTH * (mirror ? -1 : 1);
...
const getRectangleHexAuraBorder = cacheReturn(
  ...
  function (
    ...
    const lastIsSmall =
      radius === 0 ||
      (secondaryAxisSize > 1 && secondaryAxisSize % 2 === +isVariant2);
    ...
    function* generateFlatSide(sideLength, angle, startSmall) {
      ...
      const dx1 =
        Math.cos(angle + RADIANS_60 * (startSmall ? -1 : 1)) * UNIT_SIDE_LENGTH;
      const dy1 =
        Math.sin(angle + RADIANS_60 * (startSmall ? -1 : 1)) * UNIT_SIDE_LENGTH;
      const dx2 =
        Math.cos(angle + RADIANS_60 * (startSmall ? 1 : -1)) * UNIT_SIDE_LENGTH;
      const dy2 =
        Math.sin(angle + RADIANS_60 * (startSmall ? 1 : -1)) * UNIT_SIDE_LENGTH;
...
const getEllipseHexTokenSpaces = cacheReturn(
  ...
  function (primaryAxisSize, secondaryAxisSize, isColumnar, isVariant2) {
    ...
    const secondaryAxisOffset =
      Math[isVariant2 ? "ceil" : "floor"]((secondaryAxisSize - 1) / 2) *
        UNIT_SIDE_LENGTH *
        1.5 +
      UNIT_SIDE_LENGTH;
...
const getTrapezoidHexTokenSpaces = cacheReturn(
  ...
  function (primaryAxisSize, secondaryAxisSize, isColumnar, isVariant2) {
    ...
    const secondaryAxisOffset = isVariant2
      ? UNIT_SIDE_LENGTH + (secondaryAxisSize - 1) * UNIT_SIDE_LENGTH * 1.5
      : UNIT_SIDE_LENGTH;
    ...
    for (let i = 0; i < secondaryAxisSize; i++) {
      ...
      const secondaryPosition =
        i * (isVariant2 ? -1 : 1) * UNIT_SIDE_LENGTH * 1.5 +
        secondaryAxisOffset;
 ...
 */
function getSpacesUnderHexToken(
  x,
  y,
  width,
  height,
  ...
  switch (shape) {
    ...
    case CONST.TOKEN_HEXAGONAL_SHAPES.ELLIPSE_2:
      return getEllipseHexTokenSpaces(
        primaryAxisSize,
        secondaryAxisSize,
        isColumnar,
        shape === CONST.TOKEN_HEXAGONAL_SHAPES.ELLIPSE_2,
    ...
    case CONST.TOKEN_HEXAGONAL_SHAPES.TRAPEZOID_2:
      return getTrapezoidHexTokenSpaces(
        primaryAxisSize,
        secondaryAxisSize,
        isColumnar,
        shape === CONST.TOKEN_HEXAGONAL_SHAPES.TRAPEZOID_2,
      ).map((p) => ({ x: x + p.x * gridSize, y: y + p.y * gridSize }));

    case CONST.TOKEN_HEXAGONAL_SHAPES.RECTANGLE_1:
    case CONST.TOKEN_HEXAGONAL_SHAPES.RECTANGLE_2:
      return getRectangleHexTokenSpaces(
        primaryAxisSize,
        secondaryAxisSize,
        isColumnar,
        shape === CONST.TOKEN_HEXAGONAL_SHAPES.RECTANGLE_2,
      ).map((p) => ({ x: x + p.x * gridSize, y: y + p.y * gridSize }));
